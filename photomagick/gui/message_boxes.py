...
class DialogQuestion(MyMessageDialog):
	...
	def __init__(self, parent, primary_text, secondary_text, affirmative_button,
			cancel_button, icon=wx.ART_QUESTION):
	...
	def _create_buttons(self, buttons):
		...
		grid.Add((1, 1), 1, wx.EXPAND)
		if isinstance(self.cancel_button, (str, unicode)):
			btn = wx.Button(self, wx.ID_CANCEL, self.cancel_button)
		else:
			btn = wx.Button(self, self.cancel_button, '')
			self.Bind(wx.EVT_BUTTON, self._on_btn_no, btn)
		grid.Add(btn)
		grid.Add((6, 6))
		if isinstance(self.affirmative_button, (str, unicode)):
			btn = wx.Button(self, wx.ID_YES, self.affirmative_button)
		else:
			btn = wx.Button(self, self.affirmative_button, '')
			self.Bind(wx.EVT_BUTTON, self._on_btn_yes, btn)
		grid.Add(btn)
		btn.SetDefault()
		return grid
...
def message_box_question(parent, primary_text, secondary_text,
		affirmative_button=None, cancel_button=None, icon=None):
